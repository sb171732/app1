...
// }
  Future<void> loadMenu() async {
  final response = await http.get(Uri.parse('http://192.168.76.55:5000/menu'));
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: <Widget>[
                            Expanded(
                              child: Text(
                                "Текст, который должен быть независимо от товаров",
                                style: TextStyle(fontSize: 20),
                              ),
                            ),
                            TextButton(
                              onPressed: () {
                                // Обработка нажатия на кнопку, если это необходимо
                              },
                              style: TextButton.styleFrom(
                                backgroundColor: const Color.fromARGB(255, 170, 126, 126),
                              ),
                              child: Text("Кнопка", style: TextStyle(color: Colors.black)),
                            ),
                          ],
                        ),
                      ),
...
class DishDetails extends StatelessWidget {
 ...
 DishDetails({required this.dish, required this.isFromCart, required this.pageController, required this.onClose});
...
class _AnimatedDishDetailsState extends State<AnimatedDishDetails>
  ...
  Widget build(BuildContext context) {
    ...
    return SlideTransition(
      ...
      child: SafeArea( // Обернул в SafeArea
        child: Scaffold(
...
class MenuKFCwidget extends StatelessWidget {
 ...
 final void Function(CartState) onTapCallback;
  ...
  width: imageWidth,
  height: imageHeight,
  placeholder: (context, url) => CircularProgressIndicator(), // Опционально: отображение индикатора загрузки
  errorWidget: (context, url, error) => Icon(Icons.error), // Опционально: отображение иконки ошибки
),
...
class CartKFCwidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: SingleChildScrollView( // Обернул ваш виджет в SingleChildScrollView
        child: Column(
          children: [
            Container(
            ...
            Column(
              ...
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    IconButton(onPressed: () => onUpdateQuantity(1), icon: Icon(Icons.add)),
                    Text(quantity.toString()), // Отображение количества товара
                    IconButton(onPressed: () => onUpdateQuantity(-1), icon: Icon(Icons.remove)),
                  ],
                ),
              ],
            ),
